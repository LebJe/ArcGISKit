...
/// A [User](https://developers.arcgis.com/rest/users-groups-and-items/user.htm) resource represents a registered user
/// of the portal.
///
/// Personal details of the user, such as e-mail and groups, are returned only to the user or the administrator of the
/// user's organization.
///
/// A user is not visible to any other user (except the organization's administrator) if their access setting is set to
/// "private."
public struct User: Codable, Equatable {
	...
	/// The total storage amount allowed for the user's organization or subscription in Byte. Usually 2TB for organization,
	/// 2GB for non-organization.
	...
	/// culture is en or es).
	/// See [Languages](https://developers.arcgis.com/rest/users-groups-and-items/languages.htm#GUID-F2075D30-8644-4A62-915F-D21A4CEB4587)
	/// for supported formats. It will inherit from
	/// [organization](https://developers.arcgis.com/rest/users-groups-and-items/portal-self.htm) `cultureFormat` if
	/// undefined.
	public var cultureFormat: String?

	/// The user preferred region, used to set the featured maps on the home page, content in the gallery, and the default
	/// extent of new maps in the Viewer.
	...
	/// An array of groups the user belongs to. See
	/// [Group](https://developers.arcgis.com/rest/users-groups-and-items/group.htm) for properties of a group.
	...
	/// content within that user's content space.
	///
	/// - Parameter name: The name of the folder to create.
	/// - Throws: `AGKRequestError`
	/// - Returns: The ID of the created folder
	///
	/// The create user folder operation (POST only) is available only on the user's root folder. Multilevel folders are
	/// not supported.
	/// The user provides the title for the folder, which must be unique to that user. The folder ID is generated by the
	/// system.
